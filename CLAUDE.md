...
| 欄位 | 本案設定 |
|---|---|
| Repo 名稱 | `ai-native-game-1` |
| 遊戲名稱 | 見好就收（v0.1.0，2026-09-13 製作人真機驗收通過） |
| 類型 | 桌遊改編的單頁網頁遊戲 |
| 玩法模式 | 單機（對 AI）或同一台裝置輪流（hot-seat）；無連線、無金流 |
| 交付形式 | 單一 `index.html`（由 `build/` 打包 `src/` 產生），零依賴、可離線 |
...
```
src/engine/       純函數狀態機、規則、AI 對手      → 規則邏輯改這裡
...
| story-editor | docs/design/、src/data/ 文案欄位 | 改數值、改程式 |
...
| G2 | 企劃 | story-editor | `docs/design/` 故事、世界觀、核心迴圈 | **是**：製作人拍板企劃方向 |
